...
namespace
{
	/*bool operator == (const Gui::ViewProvider::PathShapePair& left, const Gui::ViewProvider::PathShapePair& right)
	{
...
namespace Gui
{
	...
	void ViewProvider::display(ViewerShape* shape)
	{
		...
		//TODO: будут ли равны два одинаковых по размеру и положению Shape?
		...
		/*SoSearchAction sa;
		sa.setNode(shape->getSoGroup());
		sa.setInterest(SoSearchAction::FIRST);
		sa.apply(viewer.getRootNode());
	...
	{
		/*PathShapeList::const_iterator it = pathShapeList.begin();
		for (;it!=pathShapeList.end(); it++)
			if (*it == group)
				break;
    	return it;*/
	...
	{
		/*PathShapeList::iterator it = pathShapeList.begin();
		for (;it!=pathShapeList.end(); it++)
			if (*it == group)
				break;
    	return it;*/
